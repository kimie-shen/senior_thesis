...
for n = 1:size(N_nums, 2)
    ...
    ag_exp_energies = zeros(N + 1, 1);
    for i = 0:2:(N - 1)
        n1 = i;
        ag_exp_energies(i + 1) = 2 * N^2 * (2 - cos(pi * n1 / N) - cos(pi * n1 / N));
        ag_exp_energies(i + 2) = 2 * N^2 * (2 - cos(pi * n1 / N) - cos(pi * 0 / N));
    end
    ...
    if (all_same_parity == true) % Remove from Au (odd n1)
        for i = 1:(index_au - 1)
            if (i < index_au)
                for j = 1:ceil(N / 2)
                    ...
                    % Remove edge solvable wavefunctions
                    if ((i < index_au) && (abs(edge_wave_energies(j) - elevels_au(i, 1)) < tolerance))
                        % Add to elevel_au_solv
                        elevels_au_solv(index_au_solv, :) = elevels_au(i, :);
                        index_au_solv = index_au_solv + 1;
            ...
            for i = 1:(index_b3g - 1)
                if (i < index_b3g)
                    for j = 1:ceil(N / 2)
                        ...
                        % Remove edge solvable wavefunctions
                        if ((i < index_b3g) && (abs(edge_wave_energies(j) - elevels_b3g(i, 1)) < tolerance))
                            % Add to elevel_b3g_solv
                            elevels_b3g_solv(index_b3g_solv, :) = elevels_b3g(i, :);
                            index_b3g_solv = index_b3g_solv + 1;
            ...
            for i = 1:(index_b3u - 1) % Remove from B3u
                if (i < index_b3u)
                    for j = 1:ceil(N / 2)
                        ...
                        % Remove edge solvable wavefunctions
                        if ((i < index_b3u) && (abs(edge_wave_energies(j) - elevels_b3u(i, 1)) < tolerance))
                            % Add to elevel_b3u_solv
                            elevels_b3u_solv(index_b3u_solv, :) = elevels_b3u(i, :);
                            index_b3u_solv = index_b3u_solv + 1;
...
ylim([ylow_solv, yhigh_solv])
set(figure(index_n + 2),'position',[0,100,800,300])

...
for n = 1:size(N_nums, 2)
    ...
    for i = 1:2
        ...
        index = index_from_coord(l1 * N, l1 * N, faces(i), N, l1, l2, total_sites);
        H(index, index) = H(index, index) + corner_potential;
    end
    ...
    for i = 1:2
        ...
        index = index_from_coord(l1 * N, l2 * N, faces(i), N, l1, l2, total_sites);
        H(index, index) = H(index, index) + corner_potential;
    end
    ...
    for i = 1:2
        ...
        index = index_from_coord(l2 * N, l1 * N, faces(i), N, l1, l2, total_sites);
        H(index, index) = H(index, index) + corner_potential;
    end
    ...
    if (l1_even == false) % l1 is odd
        ...
        solvable_prop(index_size, :) = [size(elevels_a1g, 1), size(elevels_a1u, 1)] / (total_sites);
    end
...
ylim([0.248, 0.253])
ylabel('Proportion')
saveas(gcf, [folderName '/proportion_irreps_w/solvable.jpeg']);

...
solvable_prop = zeros(size(N_nums, 1), 6);
...
for n = 1:size(N_nums, 2)
    ...
    for i = 1:size(energy_levels_rounded, 1)
        ...
        elseif (isequal(traces, [4, 0, 4, 0, 0])) % Accidental degeneracy A1g + A2g + A1u + A2u
            elevels_a1g(index_a1g, :) = energy_levels_rounded(i, :);
            index_a1g = index_a1g + 1;
            elevels_a2g(index_a2g, :) = energy_levels_rounded(i, :);
            index_a2g = index_a2g + 1;
            ...
            elevels_a2u(index_a2u, :) = energy_levels_rounded(i, :); 
            index_a2u = index_a2u + 1;
        elseif (isequal(traces, [4, 0, -2, 0, 0])) % Accidental degeneracy Eu + Eg
            elevels_eg(index_eg, :) = energy_levels_rounded(i, :);
            index_eg = index_eg + 1;
            elevels_eu(index_eu, :) = energy_levels_rounded(i, :);
            index_eu = index_eu + 1;
        elseif (isequal(traces, [2, 0, 2, 2, 0])) % Accidental degeneracy A1g + A2g
            elevels_a1g(index_a1g, :) = energy_levels_rounded(i, :);
            index_a1g = index_a1g + 1;
            elevels_a2g(index_a2g, :) = energy_levels_rounded(i, :);
            index_a2g = index_a2g + 1;
        %elseif (isequal(traces, [2, 0, 2, -2, 0])) % Accidental degeneracy A1u + A2u
            %elevels_a1u(index_a1u, :) = energy_levels_rounded(i, :);
            %index_a1u = index_a1u + 1;
            %elevels_a2u(index_a2u, :) = energy_levels_rounded(i, :);
            %index_a2u = index_a2u + 1;
        elseif (isequal(traces, [2, 2, 2, 0, 0])) % Accidental degeneracy A1g + A2u
            elevels_a1g(index_a1g, :) = energy_levels_rounded(i, :);
            index_a1g = index_a1g + 1;
            elevels_a2u(index_a2u, :) = energy_levels_rounded(i, :);
            index_a2u = index_a2u + 1;
    ...
    solvable_prop(index_size, :) = [size(elevels_a1g, 1), size(elevels_a2g, 1), size(elevels_a1u, 1), size(elevels_a2u, 1), size(elevels_eg, 1), size(elevels_eu, 1)] / (2 * (N + 1)^2);
...
prop_2d_irreps = zeros(size(size_array, 1), 1);

for i = 1:size(size_array, 1)
    total = size_array(i, 2) + size_array(i, 3) + size_array(i, 7) + size_array(i, 8) + ...
        2 * (size_array(i, 4) + size_array(i, 9)) + 3 * (size_array(i, 5) + size_array(i, 6) + size_array(i, 10) + size_array(i, 11));
    prop_1d_irreps(i) = (size_array(i, 2) + size_array(i, 3) + size_array(i, 7) + size_array(i, 8)) / total;
    prop_2d_irreps(i) = 2 * (size_array(i, 4) + size_array(i, 9)) / total;
end
...
title('Proportion of solvable states in 1D irreps')
xlabel('N')
yline(1/12, '--', '1/12')
ylim([0.07, 0.095])
ylabel('Proportion')
...
nexttile
plot(N_nums, prop_2d_irreps, 'Linestyle','-','Marker','.', 'MarkerEdgeColor', [0 0.4470 0.7410])
title('Proportion of solvable states in 2D irreps')
xlabel('N')
yline(1/6, '--', '1/6')
ylim([0.15, 0.18])
ylabel('Proportion')
...
nexttile
plot(N_nums, prop_1d_irreps + prop_2d_irreps, 'Linestyle','-','Marker','.', 'MarkerEdgeColor', [0 0.4470 0.7410])
...
set(figure(index_n + 1),'position',[0,100,1000,300])
...
function i = sigma_d_legit_index(index, N)
    ...
    if (face == 1) 
        new_face = 1;
        new_x = 2 * ((N + 1) / 2 + 1 - y_coord - 1) + 1 + mod(x_coord - 1, 2);
...
function i = inv_index(index, N)
    ...
    elseif (face == 8) 
        new_face = 1;
        new_x = 2 * ((N + 1) / 2 + 1 - y_coord - 1) + 1 - x_coord + 1;
        new_y = (N + 1) / 2 - floor((x_coord - 1) / 2);
        i = index_from_coord(new_x, new_y, new_face, N);
    end
end
